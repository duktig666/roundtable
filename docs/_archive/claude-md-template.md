...
### 执行形态默认（4 角色，DEC-005 + DEC-023）

4 个可选键，各自独立，省略全部 → 保持现状（subagent 默认）：

- **`developer_form_default`**: `inline` | `subagent`（省略默认 `subagent`）
- **`tester_form_default`**: `inline` | `subagent`（省略默认 `subagent`）
- **`reviewer_form_default`**: `inline` | `subagent`（省略默认 `subagent`）
- **`dba_form_default`**: `inline` | `subagent`（省略默认 `subagent`）

形态语义：

- `inline`：本项目该角色默认在主会话内联执行，`AskUserQuestion` 直接可用，用户全程可见，无 progress emit（主会话已观察）
- `subagent`：默认走 Task 派发子会话，context 隔离，progress 透传（DEC-004；DEC-001 D8 默认行为）

适用场景（4 角色共用判据）：

- 选 `inline`：**小项目** / 单人开发 / 紧跟过程偏好 / 平均任务 < 20k token / 单文件或单模块改动为主
- 选 `subagent`：**大型项目** / 大 refactor 频繁 / 多角色并行需求 / 团队协作 context 隔离要求 / reviewer 全仓扫 / tester 17+ suites / dba 跨库 schema

注意：

- **research 角色不在此 4 键范围**（DEC-003 非用户 trigger，由 architect skill 派发）
- 每次派发仍可覆盖：任务描述里写 `@roundtable:<role> inline` 或 `@roundtable:<role> subagent` → per-dispatch 生效
- 大任务场景下若 CLAUDE.md 声明 `reviewer_form_default: inline` 且实际任务属全仓扫，orchestrator **不做兜底拒绝**，用户自担 context 爆风险（DEC-023 §3.4）
```
...
**Q: `*_form_default` 放 `CLAUDE.md` 不是违反 DEC-001 D2「零 userConfig」吗？**
A: 不违反。DEC-001 D2 禁止的是 plugin **元协议配置**（如 agent 调度、Resource Access、Escalation schema 等 plugin 内部行为），这些必须硬编码在 prompt 本体。而 4 个 `*_form_default` 键是**业务偏好**——"本项目开发者想不想看到该角色的中间过程"是项目级选择，和 `critical_modules` / `设计参考` / `工具链覆盖` 同属一类，放 `CLAUDE.md` 合规（DEC-023 §5 Q4）。

**Q: 完全不写 `角色偏好` section 会怎样？**
A: 零影响，等价于当前默认行为——4 角色均走 subagent 派发（DEC-001 D8 + DEC-005 + DEC-023 默认）。只有明确想切 `inline` 或显式声明保持 `subagent` 时才填。
