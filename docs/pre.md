...
---
# roundtable 简化版本后问题
我发现优化版本后 workflow 的phase状态流转 没有tg发我 ，全部都在终端执行
DEC 编号的机制我不是已经删除了，怎么现在还在做？
design-doc template 和 exec-plan template 定义了模版内容，是否真的好，限制了agent的输出内容。但是agent自己发挥也不一定好，怎么取舍？或者说现在的模板是否合理？包括分析阶段产出的模板
关键决策如果是tg消息AskUserQuestion唤起，会阻塞tg流程的体验。