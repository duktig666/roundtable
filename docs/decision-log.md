...
- **影响范围**: `commands/workflow.md` 顶部新增 Step -1 Bootstrap（~5 行）+ Step 5 Escalation 渲染分支（~3 行）；`commands/bugfix.md` ref workflow.md（~4 行）；`skills/architect/SKILL.md` + `skills/analyst/SKILL.md` 加条件分支各 ~5 行；`README.md` + `README-zh.md` §决策模式章节各 ~10 行；`docs/design-docs/decision-mode-switch.md` + `docs/exec-plans/active/decision-mode-switch-plan.md` + `docs/testing/decision-mode-switch.md` 新建；`docs/decision-log.md` 本条置顶；`docs/INDEX.md` + `docs/log.md` 追加。**不改** 5 agent prompt 本体；**不改** DEC-002 / DEC-003 / DEC-004 / DEC-005 / DEC-006 / DEC-007 / DEC-008 / DEC-011 / DEC-012 任何 Accepted 条款；**不改** Phase Matrix / Step 4 并行判定树 / critical_modules 机械触发；**不改** target CLAUDE.md 业务规则边界。运行时：远程 / TG / CI 前端下 text 模式生效 workflow 不再阻塞；本地用户可 env 或 CLI arg 切换偏好。**post-fix 2026-04-20**：tester F1/F2/F3/F5/E1/E2/E4/E5/E7 全部 inline 回填 design-doc（§2.1 非法值 fallback / §2.1a timeout 非目标 / §3.1 canonical schema + 推荐标记统一 / §3.1.1 多块串行 emit / §3.1.2 id 命名空间 / §3.6 歧义处理 4 层）+ 3 处 prompt 本体 canonical 行格式统一 `<letter>（★ 推荐）：<label> — <rationale> / <tradeoff>`；不新开 DEC（append-only，属 DEC-013 scope 内 clarification）。**post-fix 2026-04-20（issue #38）**：追加 §3.1a Active channel forwarding —— TG/CI 远程前端 active inbound channel 下 orchestrator 与 skill emit `<decision-needed>` 必须同步调 channel reply 工具转发同块体；3 处 prompt 本体 inline 加 ~3 行（`commands/workflow.md` Step 5 text 分支 / `skills/architect/SKILL.md` text 段 / `skills/analyst/SKILL.md` text 段）；不新开 DEC（append-only clarification，不改 DEC-013 决定 8 "展现与接收解耦" 边界：转发是 orchestrator 内部动作，不是前端硬编码）。**post-fix 2026-04-21（issue #48）**：扩展 §3.1a 转发语义到 5 类 orchestrator-emitted 事件 —— (a) Step 0 context + Step 1 size judgment (b) A 类 producer-pause 3 行 summary (c) role completion ≤200 字 digest (d) C 类 verification-chain 🔄 handoff (e) auto_mode 4 audit events (`auto-go` / `auto-accept` / `auto-pick` / `auto-halt`)；落点**仅** `commands/workflow.md` + `commands/bugfix.md`（orchestrator-only，skill prompt 零改动）；不转发普通对话 / FAQ / 调试输出 / subagent 工具 echo；sticky channel 语义与 §3.1a 同；两类转发规则（`<decision-needed>` / 5 新类）并存不冲突；不新开 DEC（append-only clarification，不改 DEC-013 决定 8 边界）；详见 `docs/design-docs/tg-forwarding-expansion.md`
...
- **影响范围**: `commands/workflow.md` §Step 6 规则 1 重写 + Phase Matrix 新增 Stage 9；`CLAUDE.md` §critical_modules 条目 6 描述扩展；`docs/claude-md-template.md` 同步；可选 README.md 一句话提及。不影响 `commands/bugfix.md`、`agents/*`、`skills/*`、DEC-001 ~ DEC-005。**post-fix 2026-04-21（issue #30）**：A 类 producer-pause 菜单协议澄清 —— 菜单必须穷举 + 禁止 silent default + Q&A `问: ...` 循环（orchestrator 重 emit 菜单直到 `go` / `调` / `停`）+ architect `go` 拆 `go-with-plan` / `go-without-plan: <理由>`（豁免理由必填落盘）；落点 `commands/workflow.md` Step 6.1 A 类块 + `skills/architect/SKILL.md` 阶段 3 + `skills/analyst/SKILL.md` §工作流程 step 8；不新开 DEC（append-only clarification）；不改 DEC-006 A/B/C 三分、B 类 approval-gate、C 类 verification-chain 任何语义；详见 `docs/design-docs/phase-end-approval-gate.md`。**post-fix 2026-04-21（issue #26）**：Stage 9 Closeout 结构化流程规范 —— `go` 触发 orchestrator emit closeout bundle（commit msg + PR body + follow-up issues 3 section；Conventional Commits 格式 + 本批 PR 稳定模板 + 自动提取 tester/reviewer non-blocking findings 作 follow-up issue 草稿）+ `go-all` / `go-commit` / `go-pr` / `go-issues` / `skip-*` 用户驱动精细执行；memory `feedback_no_auto_push` / `feedback_no_auto_pr` 硬边界优先于 §Auto-pick（auto_mode 只 auto-推进 bundle 生成不 auto-执行 git/gh）；落点 `commands/workflow.md` Step 6.1 A 类块 Stage 9 变体 + §Auto-pick 例外 + `commands/bugfix.md` ref；不新开 DEC（append-only clarification）；详见 `docs/design-docs/closeout-spec.md`
