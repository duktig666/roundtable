...
Bugfix 任务通常很小（单 bug hotfix / 1-2 文件），本 command 对 4 角色（developer / tester / reviewer / dba）**均偏向 `inline`**（与 workflow 相反；DEC-023 扩展）。按优先级（`<role>` ∈ `{developer, tester, reviewer, dba}`）：

1. **用户显式声明**：`@roundtable:<role> inline` 或 `@roundtable:<role> subagent` → 直接遵从
2. **Target CLAUDE.md**：`<role>_form_default: inline | subagent` 任一声明均遵从（4 键：`developer_form_default` / `tester_form_default` / `reviewer_form_default` / `dba_form_default`；对称处理是 DEC-009 决定 9 对 DEC-005 §3.4.2 per-project 三级切换的 follow-through 修正 + DEC-023 扩展）
3. **AskUserQuestion**：前两条都不决定时，按 architect Option Schema 调用：
   - 小 bug（单文件 + 简单逻辑）→ `inline` = recommended
   - 跨多模块 / 需大范围重构 → `subagent` = recommended

三级切换机制（用户 > CLAUDE.md > AskUserQuestion）与 workflow §Step 6b 一致；差异仅在**默认偏向 inline**。`research` 仍 subagent-only（DEC-003 正交）。
