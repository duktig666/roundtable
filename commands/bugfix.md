...
Tier 2 优先级 critical > DEC > 生产事故；用户显式"降级到 Tier 1"可 override critical（orchestrator emit 一次警示确认）。**捷径**：summary ≤3 句 且 单文件 且 ≤80 LOC → 直接 Tier 0 无问询。**灰区**（未命中 Tier 2 也不触捷径）：emit 一次 `<decision-needed>` 选 0/1/2，按"文件数/LOC/跨模块度"挂 ★。
...
1. 派发 developer 前 orchestrator 把 `{slug}.tier` 写入 session 记忆
...
**Closeout bundle**：bugfix Stage 9 等价 A 类终点，沿用 `commands/workflow.md` Stage 9 Closeout 变体（issue #26）—— `go` 触发 commit msg + PR body + follow-up issues 3 section bundle，`go-all` / `go-commit` / `skip-*` 等用户驱动；memory `feedback_no_auto_*` 硬边界。
