...
**Phase & audit forwarding**：沿用 `commands/workflow.md` Step 5b 完整规则 —— active channel 下 context detection 结果（a）/ role completion digest（c）/ C 类交接（d）/ auto_mode 4 audit 事件（e）强制同步转发，`markdownv2` 结构化。**事件类 b（A 类 producer-pause）不适用 bugfix**（bugfix 流程无 A 类 pause）。详见 `docs/design-docs/tg-forwarding-expansion.md`。
...
复用 `/roundtable:workflow` Progress Monitor 机制。
...
**Gate + env opt-out**：仅 `run_in_background: true` 且 `ROUNDTABLE_PROGRESS_DISABLE` 未设为 `1` 的派发触发本 Step。前台派发 / env opt-out / 4 角色 inline 形态（developer / tester / reviewer / dba；见 Step 3 §Role Form Selection）均 skip Monitor setup 并不注入 4 progress 变量；subagent 按其 `## Progress Reporting` fallback 静默降级。并行批每个 `Task` 独立评估。

**执行**：Gate 通过且未 opt-out → 按 `commands/workflow.md` Progress Monitor Setup 段执行 Bash 准备 / Monitor 启动 / 4 变量注入 / Lifecycle。

**Bugfix 差异**：默认 fan-out 窄（通常 1 个 developer + 可选 reviewer / dba / tester），每个后台派发独立跑一次。任一角色 inline 分支整体 skip 本 Step。
...
### Role Form Selection（bugfix 偏向 inline）

Bugfix 任务通常很小（单 bug hotfix / 1-2 文件），本 command 对 4 角色（developer / tester / reviewer / dba）**均偏向 `inline`**（与 workflow 相反）。按优先级（`<role>` ∈ `{developer, tester, reviewer, dba}`）：

1. **用户显式声明**：`@roundtable:<role> inline` 或 `@roundtable:<role> subagent` → 直接遵从
2. **Target CLAUDE.md**：`<role>_form_default: inline | subagent` 任一声明均遵从（4 键：`developer_form_default` / `tester_form_default` / `reviewer_form_default` / `dba_form_default`；对称处理是 DEC-009 决定 9 对 DEC-005 §3.4.2 per-project 三级切换的 follow-through 修正）
3. **AskUserQuestion**：前两条都不决定时，按 architect Option Schema 调用：
   - 小 bug（单文件 + 简单逻辑）→ `inline` = recommended
   - 跨多模块 / 需大范围重构 → `subagent` = recommended

三级切换机制（用户 > CLAUDE.md > AskUserQuestion）与 workflow 一致；差异仅在**默认偏向 inline**。`research` 仍 subagent-only。

**Form → 派发路径**：
- `inline`：orchestrator `Read` `agents/<role>.md` 在主会话执行；`AskUserQuestion` 直接可用；skip Step 0.5 Monitor setup
...
### Postmortem 硬约束（Tier 2，含 orchestrator 执行锚点）
...
4. developer 补写只读 `{docs_root}/bugfixes/[slug].md` postmortem 模板，不改其他产出
...
每次派发 reviewer / dba / tester 时先按 §Role Form Selection 判定形态：`inline` 直接主会话执行，skip Step 0.5；`subagent` 按 Step 0.5 模板重生成 `dispatch_id` + `progress_path` + Monitor。
...
**前缀白名单**：沿用 workflow §Step 8 + `fix-rootcause`（postmortem 文件不走 batching）。
