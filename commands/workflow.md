...
在整个派发生命周期维护本 matrix；每次 phase 切换或用户询问进度时重新报告。**渲染 locus = orchestrator**（DEC-024，与 `tg-forwarding-expansion.md §D1` 5 类 orchestrator-emitted 事件同一落点纪律）；不下放 subagent。re-emit 义务绑定到 §Step 6 A/B/C 三类 phase gating；TG channel 下 11 行 ASCII 伪表（code fence 无语言标签，列宽 19/9/6 byte-exact）随附事件类 b/d/e 尾段转发（DEC-027 Refines DEC-024 决定 4；不新增事件类）。
...
| b | A 类 producer-pause 3 行 summary（`✅ <role> 完成 / 产出 / go \| 调 \| 问 \| 停`）| Step 6.1 A 类 | `markdownv2`：粗体标题 + 反引号路径 + 反引号操作词；**尾段随附** 11 行 ASCII 伪表 Matrix 快照（独立 code fence，无语言标签；列宽 Stage 19 / Role 9 / Status 6 byte-exact；DEC-027 Refines DEC-024 决定 4）|
...
- **A. producer-pause** —— 阶段以用户可消费产物结尾。Orchestrator 给 summary 并**停下不调用任何工具**等用户下一条；**菜单穷举 + 禁止 silent default**：
  ...
  **菜单穷举原则**：列全可能动作；"跳过某产出" = deliberate choice 必须显式说理 + 落盘；禁止 silent default。**转发**：active channel 下按 Step 5b 事件类 b 转发（`markdownv2` 结构化）。**Phase Matrix re-emit**（DEC-024 + DEC-027）：orchestrator 在 3 行 summary 之后、"请阅读后告诉我" 之前**必须**终端渲染 9 行 matrix（当前 stage `🔄 → ✅`；若下一 stage 已确定则 `⏳ → 🔄`）；TG channel sticky 下事件类 b reply 尾段随附 11 行 ASCII 伪表（独立 code fence，列宽 Stage 19 / Role 9 / Status 6 byte-exact）。

- **B. approval-gate** —— 强方向性锁。**唯一** B 类是 Design confirmation（Stage 4）。按 Option Schema 调 `AskUserQuestion`（Accept / Modify ... / Reject），每选项带 `rationale` + `tradeoff` + 可选 `recommended`。**Phase Matrix re-emit**（DEC-024 + DEC-027）：orchestrator 在 emit `AskUserQuestion` 之前**必须**终端渲染 9 行 matrix（Stage 3 `🔄 → ✅`、Stage 4 `⏳ → 🔄`）；TG channel sticky 下若 `auto_mode=true` 走 `auto-accept` 由事件类 e 尾段携带 11 行伪表，否则 `<decision-needed>` 块按 §3.1a 转发不追加（B 类决策本身即进度信号，避免冗余）。

- **C. verification-chain** —— 内部交接自动推进，emit 一行 `🔄 X 完成 → dispatching Y (critical_modules hit: [...])`。`critical_modules` 驱动的强制派发仍属 C（CLAUDE.md 已预授权）。Critical findings / `<escalation>` / lint+test 失败立即打断走 Step 5。**发 C 类交接前必须扫 final message `<escalation>` 标签**；存在则暂停自动推进走 Step 5。**转发**：active channel 下先按 Step 5b 事件类 c 独立 reply 转发 role completion digest（≤200 Unicode codepoints），再按事件类 d（与同 tick 触发的 e 合并）转发。Stage 1 Context 已由事件类 a 覆盖，不再重发 d（避免与首次 `🔄` 重叠）。**Phase Matrix re-emit**（DEC-024 + DEC-027）：orchestrator 在 emit `🔄 X → Y` handoff 前**必须**终端渲染 9 行 matrix（上游 stage `🔄 → ✅`、下游 stage `⏳ → 🔄`）；TG channel sticky 下事件类 d reply 尾段随附 11 行 ASCII 伪表（独立 code fence，列宽 Stage 19 / Role 9 / Status 6 byte-exact）。Stage 1 Context（由事件类 a 覆盖，不走 d）同步 emit 矩阵首行快照（全 ⏳ 初态 → Stage 1 `🔄`）。
...
5. 每次 phase transition 由 orchestrator 更新 matrix 并重 emit 9 行全量表格到终端；TG channel sticky 下按 §Step 6 A/B/C 三类绑定随事件类 b/d/e 尾段携带 11 行 ASCII 伪表（列宽 Stage 19 / Role 9 / Status 6 byte-exact；DEC-024 + DEC-027）
