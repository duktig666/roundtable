...
涉 DB 变更：developer 后派发 `@roundtable:dba`。规模模糊时 `AskUserQuestion` medium/large 两选项（每项 `rationale` + `tradeoff`）；`auto_mode=true` 见 §Auto-pick。**转发**：active channel 下判定结果随 Step 0 context 同块转发（Step 5b 事件类 a）。
...
2. **`auto_mode=true` 时先走 §Auto-pick 通用规则**（DEC-015，优先于 decision_mode 判定）：命中 recommended → auto-pick 注入重派；无 recommended → auto-halt fallback 步骤 3
...
## Step 5b: Phase & Audit Forwarding（DEC-013 §3.1a 扩展 / issue #48）
...
| 类 | 事件 | 来源 | 格式 |
|---|------|------|------|
| a | Step 0 context detection 结果 + Step 1 size/pipeline 判定 | orchestrator Step 0/1 | ``` 代码围栏零转义 |
| b | A 类 producer-pause 3 行 summary（`✅ <role> 完成 / 产出 / go \| 调 \| 问 \| 停`）| Step 6.1 A 类 | `markdownv2`：粗体标题 + 反引号路径 + 反引号操作词 |
| b-9 | **Stage 9 Closeout bundle 特例**（issue #26）：3 section `=== commit message === / === PR body === / === follow-up issues ===` 可 >3500 字符 | Step 6.1 A 类 Stage 9 变体 | ``` 围栏零转义整体包裹；若 >3500 chars 拆 2-3 reply（commit 独立 / PR body 独立 / issues 独立），避免超 TG Bot API 4096 字符上限 |
...
| e | auto_mode 4 audit 事件（`🟢 auto-go` / `🟢 auto-accept` / `🟢 auto-pick` / `🔴 auto-halt`）| §Auto-pick 触发点 | `markdownv2` 单行 或 多事件 ``` 批量围栏 |
...
- 事件类 a + Step 1 size 判定 **合并为单次**围栏转发（若中途用户 FAQ 插入则 Step 1 判定另发）
- 事件类 b 独立（A 类 producer-pause 3 行模板自成一体）

**格式按事件类硬绑定**（F4 澄清）：以表格格式列为准（a=围栏 / b=c=d=markdownv2 / e=单事件 markdownv2 或批量围栏）。「纯 YAML vs 混合 prose」仅为回退启发式，事件类已明定时无需临场判断。
...
- **A. producer-pause** —— 阶段以用户可消费产物结尾。Orchestrator 给 summary 并**停下不调用任何工具**等用户下一条；**菜单穷举 + 禁止 silent default**（issue #30 post-fix 2026-04-21；append-only clarification to DEC-006 §A）：
  ```
  ✅ <role> 完成。
  产出：
  - <path1> — <desc>
  ...
  **Closeout bundle 协议**（issue #26）：用户 `go` 触发 orchestrator emit **closeout bundle**（3 section）：
...
**9. §Auto-pick 通用规则**（DEC-015；`auto_mode=true` 下 A/B/内部决策点唯一权威规范）：
...
3. **Per-dispatch（AskUserQuestion）**：前两条都不适用时，按 architect Option Schema 调 `AskUserQuestion`。小任务启发式（单文件 / bug hotfix / wall time <2min / token <20k / 单模块内）命中 → `inline` = recommended；否则 → `subagent` = recommended。用户回答即最终。`auto_mode=true` 见 §Auto-pick。
