...
在整个派发生命周期维护本 matrix；每次 phase 切换或用户询问进度时重新报告。**渲染 locus = orchestrator**（DEC-024，与 `tg-forwarding-expansion.md §D1` 5 类 orchestrator-emitted 事件同一落点纪律）；不下放 subagent。re-emit 义务绑定到 §Step 6 A/B/C 三类 phase gating；TG channel 下单行进度条随附事件类 b/d/e 尾段转发，不新增事件类。
...
**HARD-GATE**：会话启动时 `hooks/session-start` 已把 `ROUNDTABLE_AUTO` raw 值注入 session context 的 `<roundtable-preflight>` 块，详见 `docs/design-docs/orchestrator-bootstrap-hardening.md`。本步解算前先从 session context 读该块；未见时回落到 `env ROUNDTABLE_AUTO` 直读并向用户报告 hook 缺失。**不要**把任何更高层 harness 的 "Auto Mode" 系统提示当作 `roundtable auto_mode=true`（两者不同层）。
...
**HARD-GATE**：同 §Step -0，`hooks/session-start` 已把 `ROUNDTABLE_DECISION_MODE` raw 值注入 `<roundtable-preflight>` 块；本步解算前先读该块。
...
**Sink 不触发**：target 项目代码 debug / 特定错误定位 / 用户偏好讨论 / 纯闲聊 / 一次性对话 / A 类 `问:` 前缀（走 menu 循环路径到 analyst slug FAQ）。

**A 类 menu 激活期间裸问机制题**（用户无 `问:` 前缀）：Step 0.5 优先 —— 回答 + sink global FAQ，**不**进入 menu 循环（menu 循环专属 `问:` 前缀用户意图）。回答完 orchestrator 重 emit 原 A 类 menu。
...
**与 A 类 `问:` 区别**：A 类 menu 的 `问: ...` 触发 skill 回派答 FAQ 后**返回菜单循环**，FAQ 条目走 **analyst slug 级** `## FAQ`（analyst 报告内）；本 Step 0.5 是 **非 A 类 menu 的** 直接提问，走 **global `{docs_root}/faq.md`**。两者互补不冲突。
...
## Step 3: Slug、角色形态与 Artifact Handoff

**7 角色形态派发表**（派发前必查此表；skill 形态不能用 `Agent(subagent_type: ...)` 激活——会触发 `Agent type not found`；inline ↔ subagent 切换细节与 research 排除见 §Step 6b）：
...
| `tester` | agent / inline（见 §Step 6b） | `Agent(subagent_type: "roundtable:tester", ...)` 或 Read `agents/tester.md` 主会话执行 |
| `reviewer` | agent / inline（见 §Step 6b） | `Agent(subagent_type: "roundtable:reviewer", ...)` 或 Read `agents/reviewer.md` 主会话执行 |
| `dba` | agent / inline（见 §Step 6b） | `Agent(subagent_type: "roundtable:dba", ...)` 或 Read `agents/dba.md` 主会话执行 |
...
任何角色的 inline form（§6b；developer / tester / reviewer / dba）根本不发 `Task`，也不进入本 Step。
...
Task 返回后 `tail -F` 空闲，默认任其自然过期（可 `MonitorStop` 显式停）。Progress 文件依赖 OS tmpfiles 清理，plugin 不 gc。并行派发 4 条件天然满足（独立 DISPATCH_ID → 独立 PROGRESS_PATH → 独立 Monitor）。
...
适用范围 = orchestrator 顶层 fuzzy 决策（Size / Dispatch mode / Developer form 三点）。**Step 4 = Task 派发并行；Step 4b = 决策并行**，两者正交。不含 architect skill 阶段 1 单问 / subagent escalation / B 类 design-confirm / A 类 producer-pause 菜单（保留串行）。
...
**失败处理**：用户回复 per-question 解析；匹配失败 / 模糊 / cancel 的 decision 单独降级重问，不回滚已答决策（ref docs/design-docs/decision-mode-switch.md §3.2）。**跨问聚合回复**（`都选推荐` / `all A` / `都 go`）：(a) `都选推荐` / `都按推荐` / `all recommended` → 每 question 独立匹配 recommended label，任一 question 缺 recommended 则整句走 §3.6 歧义澄清；(b) `都 A` / `all A` 仅当每 question 都存在 A label 且不跨问歧义时批量解析，否则降级逐问重问。**歧义重问上限 = 3（DEC-021 Refines DEC-016 §3.2）**：per-question 降级后走 `docs/design-docs/decision-mode-switch.md` §3.6 层级澄清，orchestrator **每 question 独立计数**歧义重问轮次；第 1-3 轮按 §3.6 层级澄清（不变）；**第 4 轮**开始 orchestrator emit 审计行 `🔴 halt: q<n> ambiguity retry exhausted after 3 rounds`（`<n>` = batch 内 question 1-based 索引，与 fallback 块 id `batch-<slug>-<n>-q<m>` 的 `<m>` 对齐），停止该 question 的 `AskUserQuestion` / `<decision-needed>` 重问，skill-level fallback 交回主会话等用户**自由文本**（非弹窗）继续；其他已答 / 未耗尽的 question 不受影响，沿用 D3=A per-decision 语义。计数器 session 内维护，跨 session 不持久化。审计行按 §Step 5b 事件类 e 规则（markdownv2 粗体单条 reply）转发 active channel。
...
- **Render 顺序 audit-first**：orchestrator 先 emit `🔴 auto-halt batch-<id>: no recommended option at [q_k, ...]` 审计行，后 emit N 个 fallback `<decision-needed>` 块（多块串行语义）
- **转发 fan-out 1 audit + N blocks**：active channel sticky 下，audit 行按 §Step 5b 事件类 e 规则单条 `markdownv2` 粗体 reply；N 个 `<decision-needed>` 块按 §3.1a 逐块独立 pretty markdownv2 reply，保留 `id` / `question` / `option label` 三字段不改写。共 `1 + N` reply，不合并单 payload
...
3. **按 `decision_mode` 分支**：
   ...
   - `text` → 渲染 `<decision-needed id="esc-<slug>-<n>">` 块到对话流（canonical schema 见 docs/design-docs/decision-mode-switch.md §3.1）；options 行 `<letter>（★ 推荐）：<label> — <rationale> / <tradeoff>`（无推荐省 `（★ 推荐）`）；渲染前校验 option 必填字段，缺失时用 `(未提供)` 占位不静默丢弃；**多 escalation 串行 emit**（§3.1.1）；emit 后 pause 等用户自由文本回复；orchestrator LLM fuzzy 解析（`A` / `选 A` / `go with A` / `选 B 但加 X` 皆可），歧义按 §3.6 层级澄清
     - **Active channel forwarding**（§3.1a 语义等价松弛）：若本 session inbound prompt 含 `<channel source="<plugin>:<name>" chat_id="..." ...>` 标签，或该 channel 的 reply 工具在本 session 内曾调用过（sticky 语义，不按轮次窗口衰减），每次 emit `<decision-needed>` 块**必须**同步调该 channel reply 工具转发**语义等价**的 pretty 渲染——人类可读 markdownv2 结构化（粗体 question 标题 / A-B-C option 行含 `★` 推荐标识 / rationale / tradeoff 缩进 bullet / 末尾小字 id footer 作 debug 锚点），保留 `id` / `question` / `option label` 三字段不改写（防 LLM 漂移）。**不再转发 raw YAML 块**——终端 stdout 仍 emit 原 `<decision-needed>` YAML 供 orchestrator fuzzy parse 与日志回放，TG/remote channel 只收 pretty 渲染。检测不到远程 channel（纯终端 session）→ 不调 reply，行为与现状一致。只在 emit `<decision-needed>` 时触发，普通对话 / phase summary / FAQ 不在本规则范围（out of scope，另议）。
...
| 类 | 事件 | 来源 | 格式 |
|---|------|------|------|
| a | Step -0/-1 pre-flight echo（若非 `<unset>` 默认值）+ Step 0 context detection 结果 + Step 1 size/pipeline 判定 | orchestrator Step -0/-1/0/1 | `markdownv2` 结构化：粗体标题 + 反引号字段值 + bullet 清单（DEC-022） |
| b | A 类 producer-pause 3 行 summary（`✅ <role> 完成 / 产出 / go \| 调 \| 问 \| 停`）| Step 6.1 A 类 | `markdownv2`：粗体标题 + 反引号路径 + 反引号操作词；**尾段随附** `*Phase*: \`1✅ · 2⏳ · …\`` 单行 Matrix 快照（DEC-024，事件类 b/d/e 共规则）|
...
| d | C 类 verification-chain 交接一行（`🔄 X 完成 → dispatching Y (critical_modules hit: [...])`）| Step 6.1 C 类 | `markdownv2`：粗体角色名 + 反引号 critical_modules；**尾段随附** `*Phase*: \`…\`` 单行 Matrix 快照 |
| e | auto_mode 4 audit 事件（`🟢 auto-go` / `🟢 auto-accept` / `🟢 auto-pick` / `🔴 auto-halt`）| §Auto-pick 触发点 | `markdownv2` 单行 或 多事件 ``` 批量围栏。batch auto-pick 事件合并单 ``` 围栏；非 batch 单事件仍 markdownv2 粗体。**DEC-020 auto-halt text fallback**：batch 缺 recommended 触发 auto-halt 时，audit 行 markdownv2 单行 reply 先发，随后 N 个 `<decision-needed id="batch-<slug>-<n>-q<m>">` 块按 §3.1a 逐块独立 pretty markdownv2 reply（共 `1 + N` reply，不合并）。**伴 phase 切换**（`auto-accept` B 类 / `auto-go` A 类）**尾段随附** `*Phase*: \`…\`` 单行 Matrix 快照；`auto-pick`（Step 1 规模 / Step 5 escalation / Step 6b form 等非 phase transition 决策点）与 `auto-halt` 不追加。|
...
**边界**：转发仍是 orchestrator 内部动作，不硬编码前端。不改 skill/agent prompt 本体（§3.1a 原有 forwarding 规则 —— `commands/workflow.md` Step 5 text 分支 / `skills/architect/SKILL.md` text 段 / `skills/analyst/SKILL.md` text 段 共 3 处 prompt 本体 —— 继续生效）。详见 `docs/design-docs/tg-forwarding-expansion.md`。
...
## Step 5c: Skill→Orchestrator Handoff Checklist

skill 返回后 orchestrator tick 重启时**按序**执行下列 6 条 action（A 类 producer-pause 正常返回路径专属；`<escalation>` 走 §Step 5，agent 返回 C 类 verification-chain 由 §Step 6.1 C 类主管，不走本 checklist）：

1. **[flush]** Step 8 log.md flush（`log_entries:` 无新条目则 skip-if-no-new；详见 §Step 8）
2. **[sync]** Step 7 INDEX.md sync（`created[]` 空则 skip-if-no-new；详见 §Step 7）
3. **[fwd-c]** Role completion digest reply（≤200 Unicode codepoints；sticky channel 必 fire，纯终端 session 降级终端 stdout；详见 §Step 5b 事件类 c）
4. **[fwd-b]** A 类 producer-pause 3 行 summary + Phase Matrix 单行进度条（sticky channel 必 fire，纯终端 session 降级终端 stdout；详见 §Step 5b 事件类 b，Stage 9 走 b-9 变体）
5. **[menu]** A 类菜单终端 emit（`go / 问 / 调 / 停` + architect / Stage 9 / Q&A 循环变体均由 §Step 6.1 主管）
6. **[pause]** 停下不调用任何工具等用户下一条（pause 语义见 §Step 6.1 A 类定义）

**Runtime enforcement**（DEC-030 P2 follow-up 生效）：每步 orchestrator emit 一条 JSONL audit 行到 `${ROUNDTABLE_AUDIT_PATH}`；P1 落盘占位，P2 merged 后 `scripts/orchestrator-compliance-check.sh` 按 `dispatch_id` 扫 miss 集合。
...
- **A. producer-pause** —— 阶段以用户可消费产物结尾。Orchestrator 给 summary 并**停下不调用任何工具**等用户下一条；**菜单穷举 + 禁止 silent default**：
  ...
  请阅读后告诉我：
    ...
    `停`
  ```
  （skill 返回后 orchestrator tick 6 条 action 检查表详见 §Step 5c Handoff Checklist）
  ...
  **菜单穷举原则**：列全可能动作；"跳过某产出" = deliberate choice 必须显式说理 + 落盘；禁止 silent default。**转发**：active channel 下按 Step 5b 事件类 b 转发（`markdownv2` 结构化）。**Phase Matrix re-emit**：orchestrator 在 3 行 summary 之后、"请阅读后告诉我" 之前**必须**终端渲染 9 行 matrix（当前 stage `🔄 → ✅`；若下一 stage 已确定则 `⏳ → 🔄`）；TG channel sticky 下事件类 b reply 尾段随附 `*Phase*: \`…\`` 单行进度条。

- **B. approval-gate** —— 强方向性锁。**唯一** B 类是 Design confirmation（Stage 4）。按 Option Schema 调 `AskUserQuestion`（Accept / Modify ... / Reject），每选项带 `rationale` + `tradeoff` + 可选 `recommended`。**Phase Matrix re-emit**：orchestrator 在 emit `AskUserQuestion` 之前**必须**终端渲染 9 行 matrix（Stage 3 `🔄 → ✅`、Stage 4 `⏳ → 🔄`）；TG channel sticky 下若 `auto_mode=true` 走 `auto-accept` 由事件类 e 尾段携带，否则 `<decision-needed>` 块按 §3.1a 转发不追加（B 类决策本身即进度信号，避免冗余）。

- **C. verification-chain** —— 内部交接自动推进，emit 一行 `🔄 X 完成 → dispatching Y (critical_modules hit: [...])`。`critical_modules` 驱动的强制派发仍属 C（CLAUDE.md 已预授权）。Critical findings / `<escalation>` / lint+test 失败立即打断走 Step 5。**发 C 类交接前必须扫 final message `<escalation>` 标签**；存在则暂停自动推进走 Step 5。**转发**：active channel 下先按 Step 5b 事件类 c 独立 reply 转发 role completion digest（≤200 Unicode codepoints），再按事件类 d（与同 tick 触发的 e 合并）转发。Stage 1 Context 已由事件类 a 覆盖，不再重发 d（避免与首次 `🔄` 重叠）。**Phase Matrix re-emit**：orchestrator 在 emit `🔄 X → Y` handoff 前**必须**终端渲染 9 行 matrix（上游 stage `🔄 → ✅`、下游 stage `⏳ → 🔄`）；TG channel sticky 下事件类 d reply 尾段随附 `*Phase*: \`…\`` 单行进度条。Stage 1 Context（由事件类 a 覆盖，不走 d）同步 emit 矩阵首行快照（全 ⏳ 初态 → Stage 1 `🔄`）。
...
**4. developer 完成后**：跑 `lint_cmd` + `test_cmd`；失败报告用户，不静默重派修复。

**5. Tester 发现业务 bug**：tester 写复现测试 + `<escalation>` 上报，不改 src；orchestrator 报告用户，用户决定派 bug-fix（典型 `/roundtable:bugfix`）。

**6. Escalation**：见 Step 5。

**7. 不自动 git**：`commit` / `push` / `branch` / `tag` / `reset` / `stash` / `git add` 只在用户显式要求时执行。

**8. §Auto-pick 通用规则**（`auto_mode=true` 下 A/B/内部决策点唯一权威规范）：
...
## Step 6b: Role Form Selection

Developer / tester / reviewer / dba 均支持 `subagent`（默认）和 `inline`（DEC-005 developer + DEC-023 三角色扩展）。research 角色仍 subagent-only。本 Step 在每次上述 4 角色派发**之前**执行。

**三级切换**（按序评估，第一个匹配胜出；`<role>` ∈ `{developer, tester, reviewer, dba}`）：

1. **Per-session（用户 prompt）**：`@roundtable:<role> inline`、`<role> 用 inline` 等 → 强制 `form = inline`
2. **Per-project（target CLAUDE.md）**：`# 多角色工作流配置` 里的可选 `<role>_form_default: inline | subagent` 作为 baseline（per-session 仍覆盖）。4 个可选键：`developer_form_default` / `tester_form_default` / `reviewer_form_default` / `dba_form_default`；省略 → `subagent`（向后零破坏）
...
**启发式同一套 4 角色共用**（不按角色差分阈值；per-role 差分推迟到 future issue）。

**执行路径**：

- **Form = `inline`**：orchestrator `Read` `agents/<role>.md` 在主会话执行（同 architect/analyst 机制）；`AskUserQuestion` 直接可用；**skip Step 3.5**（无 progress）；Resource Access 约束与 subagent 一致（tester 仍 `tests/*` Write / reviewer / dba 仍只读；relay 主路径不变）。
- **Form = `subagent`**：先执行 Step 3.5；`Task` 派发带 4 progress 变量。

**audit trail**：form 解析为 `inline` 时在 phase-gate summary 加一行 `<role> dispatched inline (trigger: <per-session | per-project | per-dispatch>)`。

**Context 风险提示**：用户 per-session 强制 `reviewer inline` / `tester inline` / `dba inline` 于大任务（全仓扫 / 17 suites / 跨库 schema）时，orchestrator **不做安全兜底拒绝**，用户自担 context 爆风险（同 developer inline 边界纪律）。

**research**：CLAUDE.md 里 4 个 `*_form_default` 键**不覆盖** research（非用户 trigger；architect skill 派发，交互模型与本 Step 正交）。
...
4. **自造 `log_entries:` YAML**：`prefix: review`（或 `test-plan` for tester / `db-review` for dba）；`操作者: orchestrator (relay for <role>)`；`files: [relay artifact path]`；`note` 末尾加 `(orchestrator relay)`
...
3. 初始化 Phase Matrix（全部 ⏳；locus = orchestrator）
4. 按规模 pipeline 激活 / 派发第一个角色
5. 每次 phase transition 由 orchestrator 更新 matrix 并重 emit 9 行全量表格到终端；TG channel sticky 下按 §Step 6 A/B/C 三类绑定随事件类 b/d/e 尾段携带单行进度条
