...
涉 DB 变更：developer 后派发 `@roundtable:dba`。规模模糊时 `AskUserQuestion` medium/large 两选项（每项 `rationale` + `tradeoff`）。`auto_mode=true` 下参照 §Auto-pick 通用规则（有 `recommended` 自动采纳；无 `recommended` → auto-halt + fallback manual 渲染）；审计 `🟢 auto-pick size=<medium|large> (auto_mode=on)`。
...
2. **按 `auto_mode` 分支**（DEC-015；参照 §Auto-pick 通用规则；优先于 decision_mode 判定）：
   - `auto_mode=true` 且 options 含 `recommended: true` → **不调 `AskUserQuestion` / 不 emit `<decision-needed>`**，直接把 recommended option 的决策事实注入 prompt 重派同一 agent；emit `🟢 auto-pick <letter> <label> (auto_mode=on, why: <why_recommended>)`
   - `auto_mode=true` 且 options 均无 `recommended` → emit `🔴 auto-halt: no recommended option at <esc-<slug>-<n>>`，fallback 走步骤 3 `decision_mode` 渲染（manual 等用户）
   - `auto_mode=false` → 直接进入步骤 3（现行行为）
3. **按 `decision_mode` 分支**（DEC-013）：
...
5. **绝不替用户决策**；agent 未给 recommended 时不擅自选（auto_mode=true 亦遵守，走 auto-halt 路径）—— `recommended` 即 agent/skill 在设计阶段的**预授权**，auto-pick 沿用预授权不等同替用户挑
...
- **A. producer-pause** —— 阶段以用户可消费产物结尾。
  - `auto_mode=false`（默认）→ 3 行 summary 并**停下不调用任何工具**等用户下一条：
    ```
    ✅ <role> 完成。
    产出：
    - <path1> — <desc>
  ...
  - `auto_mode=true`（DEC-015）→ emit `🟢 auto-go <role> ✅ (auto_mode=on)` 一行审计，**不 pause**，自动推进下一 stage（仍在 pause-point 之前执行 Step 7 / Step 8 flush）。

- **B. approval-gate** —— 强方向性锁。**唯一** B 类是 Design confirmation（Stage 4）。
  - `auto_mode=false`（默认）→ 按 Option Schema 调 `AskUserQuestion`（Accept / Modify ... / Reject；decision_mode=text 时改 emit `<decision-needed>`），每选项带 `rationale` + `tradeoff` + 可选 `recommended`。
  - `auto_mode=true`（DEC-015；参照 §Auto-pick 通用规则）→ 读 options 的 `recommended` 字段：
    - 有 `recommended: true` → 自动 Accept，emit `🟢 auto-accept <role> design (recommended: <label>, auto_mode=on)`
    - 无 `recommended` → emit `🔴 auto-halt: no recommended option at design-confirm`，fallback 沿用 manual 渲染（等用户）

- **C. verification-chain** —— 内部交接自动推进，emit 一行 `🔄 X 完成 → dispatching Y (critical_modules hit: [...])`。`critical_modules` 驱动的强制派发仍属 C（CLAUDE.md 已预授权）。Critical findings / `<escalation>` / lint+test 失败立即打断走 Step 5。**发 C 类交接前必须扫 final message `<escalation>` 标签**；存在则暂停自动推进走 Step 5。`auto_mode` **不影响 C 类行为**（本就自动推进；critical_modules 派发 / tester hard regression / lint+test failure 打断在 auto 模式下保持不变）。
...
**9. §Auto-pick 通用规则**（DEC-015；`auto_mode=true` 下所有 orchestrator 层决策点共用）：

- options 含 `recommended: true` → 直接采纳，emit `🟢 auto-pick <context> (auto_mode=on)`（`<context>` 是 `letter <label>` / `size=X` / `form=Y` 等局部标识）
- 无 `recommended` → emit `🔴 auto-halt: no recommended option at <decision_id>`，fallback 沿用本地原渲染路径（`AskUserQuestion` / `<decision-needed>` 按 `decision_mode`）
- 适用点：Step 1 规模判定 / Step 5 Subagent Escalation parse（步骤 2）/ Step 6 B 类 approval-gate / Step 6b per-dispatch form 选择
- 不适用：Step 6 A 类 producer-pause（用 auto-go 路径）/ Step 6 C 类 verification-chain（本就自动，auto 不影响）
...
3. **Per-dispatch（AskUserQuestion）**：前两条都不适用时，按 architect Option Schema 调 `AskUserQuestion`。小任务启发式（单文件 / bug hotfix / wall time <2min / token <20k / 单模块内）命中 → `inline` = recommended；否则 → `subagent` = recommended。用户回答即最终。`auto_mode=true` 下参照 §Auto-pick 通用规则自动采纳 recommended；审计 `🟢 auto-pick form=<inline|subagent> (auto_mode=on)`。
