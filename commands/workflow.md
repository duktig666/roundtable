...
**Pre-flight echo**：orchestrator 每次 `/roundtable:workflow` 入口进入 §Step 0 之前**必须**先跑下面一块 Bash，把原始 CLI/env 输入与解算后的 `auto_mode` / `decision_mode` 值 echo 出来供人/tester 眼验；stdout 连同 §Step 0 context detection 结果合入 §Step 5b 事件类 a 首块 reply 转发（见 §Step 5b 事件类表）。
...
Bash 执行后 orchestrator **紧随 stdout** 以普通 echo 格式补两行 resolved 值（orchestrator 按 §Step -0/-1 优先级在 LLM 层解算后输出，非 shell 直算——规避 CLI flag shell 不可见问题）：
...
**输入源纪律**：Bash 直读 env（`ROUNDTABLE_AUTO` / `ROUNDTABLE_DECISION_MODE`），打印 `$ARGUMENTS` 原串以捕获 CLI flag；resolved 值由 orchestrator LLM 按 §Step -0 / §Step -1 优先级（CLI > env > default，含 `--no-auto`）解算。**权威源**仍是 Task prompt prefix / skill context prefix 里的 `auto_mode: <v>` / `decision_mode: <v>` 注入行；echo 仅为**用户可见校验行**，不是下游派发的参数源。

**若两 env 均 `<unset>` 且无 CLI flag**（raw 全默认）：不向 active channel 转发独立 PREFLIGHT 段，只让 resolved 两行作为 bullet 合入事件类 a 首块 reply；终端 stdout 始终打印（见 §Step 5b 事件类 a 描述）。
...
**7 角色形态映射**（派发前必查此表；skill 形态不能用 `Agent(subagent_type: ...)` 激活——会触发 `Agent type not found`；inline ↔ subagent 切换细节与 research 排除见 §Step 6b）：
...
| b | A 类 producer-pause 3 行 summary（`✅ <role> 完成 / 产出 / go \| 调 \| 问 \| 停`）| Step 6.1 A 类 | `markdownv2`：粗体标题 + 反引号路径 + 反引号操作词；**尾段随附** `*Phase*: \`1✅ · 2⏳ · …\`` 单行 Matrix 快照（DEC-024）|
...
Developer / tester / reviewer / dba 均支持 `subagent`（默认）和 `inline`（DEC-005 developer + DEC-023 三角色扩展）。research 角色仍 subagent-only（DEC-003 非用户 trigger，正交）。本 Step 在每次上述 4 角色派发**之前**执行。
...
**启发式同一套 4 角色共用**（不按角色差分阈值；DEC-023 §3.3 明确推迟 per-role 差分到 future issue）。

**执行路径**：

- **Form = `inline`**：orchestrator `Read` `agents/<role>.md` 在主会话执行（同 architect/analyst 机制）；`AskUserQuestion` 直接可用；**skip Step 3.5**（无 progress）；Resource Access 约束与 subagent 一致（tester 仍 `tests/*` Write / reviewer / dba 仍只读；DEC-017 relay 主路径不变）。
...
**Context 风险提示**：用户 per-session 强制 `reviewer inline` / `tester inline` / `dba inline` 于大任务（全仓扫 / 17 suites / 跨库 schema）时，orchestrator **不做安全兜底拒绝**，用户自担 context 爆风险（同 DEC-005 developer inline 边界纪律，DEC-023 §3.4）。

**research**：CLAUDE.md 里 4 个 `*_form_default` 键**不覆盖** research（DEC-003 非用户 trigger；architect skill 派发，交互模型与本 Step 正交）。
...
3. 初始化 Phase Matrix（全部 ⏳；locus = orchestrator，DEC-024）
4. 按规模 pipeline 激活 / 派发第一个角色
5. 每次 phase transition 由 orchestrator 更新 matrix 并重 emit 9 行全量表格到终端；TG channel sticky 下按 §Step 6 A/B/C 三类绑定随事件类 b/d/e 尾段携带单行进度条（DEC-024）
