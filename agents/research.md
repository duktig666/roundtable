...
**Research 不 emit progress**（DEC-004 / DEC-009 决定 2 retrofit）。即便 orchestrator 注入 `{{progress_path}}` 也不调用 `Bash echo ... >> {{progress_path}}`；状态在 final message `<research-result>` / `<research-abort>` 一次性交付。
